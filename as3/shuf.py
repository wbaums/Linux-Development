...
# prints numbers for -i command
    def printNums(self, low, hi):
        ...
        for i in numarr:
            ...
            sys.stdout.write("\n")
# selects a random element from array and removes it
    def rmRand(self):
...
# same format as randline.py to accept flags/arguments
...
# create instance of shuffle class
    instance = shuffle(linearr, filename, low, hi, options.lowHi)

# check if program should repeat
    if options.rep:
        if lines == 1:
            lines = 9999999999

# checks options and creates loops
    index = 0
    if options.lowHi:
